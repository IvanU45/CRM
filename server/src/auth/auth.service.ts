...
export class AuthService {
    ...
    async refresh(req: Request, res: Response) {
        ...
        if (!refreshToken) {
            throw new UnauthorizedException("Не действительный refreshToken")
        }
    ...
    async logout(res: Response) {
        this.serCookie(res, "refreshToken", new Date(0))
    ...
    private auth (res: Response, id: string) {
        ...
        this.serCookie(res, refreshToken, new Date(Date.now() + 1000*60*60*24*7))
    ...
    private serCookie(res: Response, value: string, expires: Date) {
